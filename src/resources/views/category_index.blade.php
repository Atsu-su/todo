...
<main>
  ...
  <div class="l-container">
    <div class="category-create">
      <form class="category-create__form" action="{{ route('store') }}" method="post">
        @csrf
        <input class="category-create__input" type="text" name="name" placeholder="カテゴリーを入力してください">
        <button class="c-btn c-btn--create" type="submit">作成</button>
      </form>
    </div>
    <div class="category-list">
      <h2 class="category-list__title">カテゴリー</h2>
      <ul class="category-list__table">
        @foreach ($categories as $category)
        <li class="category-list__item">
          <form id="form-update-{{ $category->id }}" class="category-list__form-update" action="{{ route('update', $category->id) }}" method="post">
            @method('patch')
            @csrf
          ...
          <form id="form-delete-{{ $category->id }}" class="category-list__form-delete action=" action="{{ route('delete', $category->id) }}" method="post">
            @method('delete')
            @csrf
